...
{
    public class EventHistoryLogViewModel : ViewModelBase
    {
        public EventHistoryLogViewModel()
        {
        ...
        public List<EventLogModel> EventLogList { get => _eventLogList; set => Set(ref _eventLogList, value); }
        private List<EventLogModel> _eventLogList = new List<EventLogModel>();
        ...
        private void UpdateProductOrderList()
        {
            Task.Run(() =>
            {
                var data = EventLogBLL.GetPage(PageData.Page, PageData.PageSize, ref totalCount, StartDateTime, EndDateTime, TypeComboxIndex, SearchText);
                var datas = JsonConvert.DeserializeObject<List<EventLogModel>>(data);
